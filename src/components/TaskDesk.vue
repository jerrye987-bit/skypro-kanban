          ...
          <template #content>
            ...
            <template v-else>
              <Task v-for="task in tasks.filter(t => t.status === 'Без статуса')" :key="task.id">
                ...
                <template #title><h3 class="card__title">{{ task.title }}</h3></template>
          ...
          <template #content>
            ...
            <template v-else>
              <Task v-for="task in tasks.filter((t) => t.status === 'Готово')" :key="task.id">
                ...
                <template #title>
                  <!-- Исправлено: вернули правильный класс для перечеркивания -->
                  <h3 class="card__title _completed">{{ task.title }}</h3>
                </template>
                <template #date>{{ task.date }}</template>
              </Task>
            </template>
...
<style lang="scss" scoped>
  ...
  width: 100%;
  min-height: 50vh; /* Занимает половину экрана по высоте для центрирования */
  flex-grow: 1;
  ...
  .empty-icon {
    font-size: 40px; /* Размер эмодзи-иконки */
    opacity: 0.7;
  }
  ...
  p {
    ...
    font-size: 20px;
    font-weight: 500;
    color: #94A6BE; /* Приглушенный серый цвет */
    margin: 0;
